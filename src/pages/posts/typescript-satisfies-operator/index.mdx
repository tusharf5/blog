...
to assign an explicit type as shown below:
...
This is possible because `satisfies` does not change the type of variable that it's used against. It's just an extra option
to validate the value's type.
