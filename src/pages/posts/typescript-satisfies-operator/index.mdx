...
You can also use the `satisfies` operator when you create object literals without a variable assignment in which case
these objects don't have any explicit type associated with them.

For example, in the code below, because these objects are not initialized using a variable assignment,
they don't have an explicit type associated with them.
The function `saveJSON` can take in any object so we don't get any type validation from there.
...
But with the `satisfies` operator, you can add type validation to the above code.

You could also use the `as` keyword but we know how `as` can sometimes be misleading. The only other
option to get type safety in the above code is to explicitly assign these objects to a
variable with the required type as shown below:
