...
This is possible because `satisfies` does not change the type of variable that it's used with. It's just an added
option to validate a value against a specific type.

You can even use it when you create object literals without assigning them to a variable in which case these objects don't
have any explicit type associated with them. For example, in the code below we are using the same function which can take in
any unknown object to save three different types of objects (user, file and directory).

Because these objects are not initialized using a variable assignment, they don't have an explicit type associated with them.
The function `saveJSON` can take in any object so we also don't get any type validation there. We can call `saveJSON` with any value.

```typescript
function saveJSON(json: unknown) {
  // save
}

// user.ts
await saveJSON({ isActive: true, id: '12-34m2ks', isVerified: true } satisfies User);

// file.ts
await saveJSON({ name: 'file', extension: '.ts' } satisfies TSFile);

// dir.ts
await saveJSON({ name: 'file' } satisfies Dir);
```

But with the `satisfies` operator, you can add type validations in scenarios like this. There was no way before the `satisfies` operator
to validate the object's types in the above scenario. You could use the `as` keyword but we know how `as` can sometimes be misleading.

The only other option to get type safety is to explicitly assign these objects to a variable with the required type as shown below:
...
// user.ts
const user: User = { isActive: true, id: '12-34m2ks', isVerified: true };
await saveJSON(user);

// file.ts
const file: TSFile = { name: 'file', extension: '.ts' };
await saveJSON(file);

// dir.ts
const dir: Dir = { name: 'file' };
await saveJSON();
```
