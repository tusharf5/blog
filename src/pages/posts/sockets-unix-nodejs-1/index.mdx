...
When creating a socket in the listening mode you don't have to specify values for the remote IP and remote port. However, for a client socket, the remote IP and remote port must be specified during its creation.

The local (source) IP for a client socket is not determined by the application code though. Instead, the kernel selects the source IP based on the outgoing interface that will be used to connect to the other endpoint, typically a server. The local port is also assigned by the TCP (kernel) and is typically a short-lived, reusable port that is freed after the request is completed and the socket is closed.

If a host with IP `199.213.82.51` wants to connect to a server with IP `25.6.85.643` on port `8008`. If the ephemeral port assigned (by the kernel) to it is `1600`, the client-side socket pair would be something like:
...
The next step is to bind the socket to a specific IP and port using the `bind()` system call. Since this is a server socket, only the local port needs to be specified while the local IP can be set to the wild card address of `0.0.0.0`, indicating that the socket can accept connections from any network interface. The remote IP and port do not need to be specified as the socket will be in a listening state.

Afterward, the `listen()` system call must be executed to transition the socket to a listening state, enabling it to actively await incoming connection requests.

> Following creation, a socket can take one of two routes: it can either become a server socket if you issued the **`listen()`** system call or it can become a client socket if you issued the **`connect()`** system call. 
...
It's important to note that the port number `8080` is used by both the listening socket and the connected socket. This is because the kernel utilizes all four elements of the socket pair (local IP, local port, remote IP, remote port) to route a data segment and not just the port number.

When a network data segment arrives at `125.6.85.643` and port `8080` from `199.213.82.51` and port `1900` it is delivered to the connected socket otherwise it is delivered to the listening socket. 
