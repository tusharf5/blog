...
```typescript
const result = user.findAndSelect({ include: { id: true, address: true, auth: { session: { active: true } }  } });
```
...
class User {
   findAndSelect<
  const K extends NestedInclude<DatabaseRecordType>
>(arg: Arg<DatabaseRecordType, K>): Selector<DatabaseRecordType, K> {
  return arg as Selector<DatabaseRecordType, K>;
}
}

new User().findAndSelect({ include: {
    id: true,
    orgUnit: { name: true },
  }}).orgUnit.name
...
const user = new User();
const result = user.findAndSelect({ include: { id: true, orgUnit: { id: true } } });
