...
Encryption ensures data privacy, but without verification, a hacker could impersonate a legitimate website, intercepting traffic while still using encryption. 
...
encrypting the data using that public key.

The CA then verifies this information and gives you a certificate. This certificate is a file that contains the public key of your website
and some information about your website like the domain names that can be used. 
The CA **signs the certificate** (adds a digital signature) using its private key and adds it to the certificate , 
allowing browsers to verify its authenticity using the CA’s public key.
...
This structure ensures security, if an Intermediate CA is compromised, only its certificates are affected not the entire Root CA.
...
Technically there is not much difference between a Root CA and an Intermediate CA. The only difference is that the Root CA is **self-signed** and the Intermediate CA is **signed by a Root CA** or an another intermediate CA.
From an operation's perspective though, the Root CA is more secure and is only used to sign Intermediate CAs.
...
This differs from public websites like https://google.com, where certificates are signed by widely trusted Root CAs that are already built into your system or browser.
